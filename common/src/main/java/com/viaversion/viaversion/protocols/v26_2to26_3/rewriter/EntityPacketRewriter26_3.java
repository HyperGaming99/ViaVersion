...
public final class EntityPacketRewriter26_3 extends EntityRewriter<ClientboundPacket26_1, Protocol26_2To26_3> {
    ...
    protected void registerPackets() {
        ...
        protocol.appendClientbound(ClientboundPackets26_1.ADD_ENTITY, wrapper -> {
            ...
            if (entityType != null && entityType.isOrHasParent(EntityTypes26_3.PROJECTILE)) {
        ...
        protocol.registerServerbound(ServerboundPackets26_3.ATTACK, toOriginalEntityId);
    ...
    public int toOriginalEntityId(final PacketWrapper wrapper, final int entityId) {
        final ProtocolStorables26_3 storables = wrapper.user().storables(protocol);
    ...
    protected void registerRewrites() {
        ...
        filter().type(EntityTypes26_3.ABSTRACT_CUBE_MOB).addIndex(16); // baby
        filter().type(EntityTypes26_3.ABSTRACT_CUBE_MOB).addIndex(17); // age locked

        final EntityDataHandler toFakeEntityId = (event, data) -> {
            final Integer target = data.value();
            if (target != null) {
                final ProtocolStorables26_3 storables = event.user().storables(protocol);
                data.setValue(target == 0 ? storables.fakeEntityId() : target);
            }
        };
        filter().type(EntityTypes26_3.FROG).index(19).handler(toFakeEntityId); // Tongue target
        filter().type(EntityTypes26_3.FIREWORK_ROCKET).index(9).handler(toFakeEntityId); // Attached to target
